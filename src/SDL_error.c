...
int
SDL_Unsupported_REAL(const char *file, int line, const char *function)
{
    /* Diagnostic logging — enable with SDL_HINT_REPORT_UNSUPPORTED_OPERATIONS="1"
    if (SDL_GetHintBoolean(SDL_HINT_REPORT_UNSUPPORTED_OPERATIONS, SDL_FALSE)) {
        if (!function || !*function) {
            function = "???";
        }
        SDL_LogWarn(SDL_LOG_CATEGORY_ERROR,
                    "Unsupported SDL operation in %s (%s:%d)",
                    function, file, line);
    }
    */
