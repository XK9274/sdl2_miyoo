...
#define CHECK_WINDOW_MAGIC(window, retval)                                       \
    ...
    if (!(window) || (window)->magic != &_this->window_magic) {                  \
        char error_buf[256];                                                     \
        SDL_snprintf(error_buf, sizeof(error_buf),                               \
                    "Invalid window in %s: ptr=%p id=%u magic=%p expected=%p",   \
                    __func__, (void *)(window),                                  \
                    (window) ? (window)->id : 0,                                 \
                    (window) ? (window)->magic : NULL, &_this->window_magic);    \
        SDL_SetError("%s", error_buf);                                           \
        return retval;                                                           \
    }
