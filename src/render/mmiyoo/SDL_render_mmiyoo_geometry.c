...
/* SDL_MMIYOO_GEOMETRY_DIRECT_WRITE fast path for small partial-alpha fills:
 * CPU-blend a merged fill span directly into the mapped window/back-buffer
 * instead of dispatching a MI_GFX_BitBlit (+fence) for it. */
SDL_bool
MMIYOO_TryDirectBlendFill(MMIYOO_RenderData *data, const SDL_Rect *dst, Uint32 color)
{
    void *vir;
    Uint32 stride;
    int fb_w;
