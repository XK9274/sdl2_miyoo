...
#define CHECK_RENDERER_MAGIC(renderer, retval)                                   \
    if (!(renderer) || (renderer)->magic != &renderer_magic) {                   \
        SDL_LogWarn(SDL_LOG_CATEGORY_RENDER,                                     \
                    "%s: renderer=%p magic=%p expected=%p",                      \
                    __func__, (void *)(renderer),                                \
                    (renderer) ? (renderer)->magic : NULL, &renderer_magic);     \
...
#define CHECK_TEXTURE_MAGIC(texture, retval)                                     \
    if (!(texture) || (texture)->magic != &texture_magic) {                      \
        SDL_LogWarn(SDL_LOG_CATEGORY_RENDER,                                     \
                    "%s: texture=%p magic=%p expected=%p",                       \
                    __func__, (void *)(texture),                                 \
                    (texture) ? (texture)->magic : NULL, &texture_magic);        \
...
static const SDL_RenderDriver *render_drivers[] = {
    ...
    /* Must come before GLES2 below -- index=-1 picks the first driver that
     * works, and GLES2 crashes on a non-GL window. */
    &MMIYOO_RenderDriver,
#endif
...
SDL_CreateRenderer(SDL_Window *window, int index, Uint32 flags)
{
    ...
    if(!window) {
        SDL_LogWarn(SDL_LOG_CATEGORY_RENDER,
                    "SDL_CreateRenderer: window pointer is NULL");
                ...
                const SDL_Rect *srcrect, const SDL_FRect *dstrect)
{
    ...
    if (!texture) {
        SDL_LogDebug(SDL_LOG_CATEGORY_RENDER,
                     "SDL_RenderCopyF: NULL texture, skipping draw");
        return 0;
    }
    CHECK_TEXTURE_MAGIC(texture, -1);
                  ...
                  const double angle, const SDL_FPoint *center, const SDL_RendererFlip flip)
{
    ...
    if (!texture) {
        SDL_LogDebug(SDL_LOG_CATEGORY_RENDER,
                     "SDL_RenderCopyExF: NULL texture, skipping draw");
        return 0;
    }
    CHECK_TEXTURE_MAGIC(texture, -1);
