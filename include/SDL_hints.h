...
/**
 *  \brief  A variable that enables logging when SDL_Unsupported() is invoked.
 *
 *  SDL_Unsupported() is used by the backend implementations to report that a
 *  requested operation is not available. When this hint is set to "1", SDL will
 *  log the function, file, and line number that triggered the unsupported
 *  operation, which can help backend developers track down missing features.
 *
 *  This hint can be toggled on and off at runtime. The default value is "0".
 */
#define SDL_HINT_REPORT_UNSUPPORTED_OPERATIONS "SDL_REPORT_UNSUPPORTED_OPERATIONS"
